<template>
  <nav class="h-full w-full bg-white dark:bg-black flex items-center px-4">
    ...
    <!-- PROMOÇÕES -->
    <div v-if="login.loggedIn && checkApostador()">
      <div class="inline-flex relative w-fit">
        <button
          ...
          type="button"
          @click="openModalPromos=true">
            <GiftIcon class="h-8"/>
        </button>
      </div>
      
      <BaseModalPromocoes :is-open="openModalPromos" @close="openModalPromos=false" />
    </div>
    ...
    <div v-if="login.loggedIn">
      <div class="inline-flex relative w-fit">
        ...
        <button
          class="mr-2 p-1 text-rasbet-logo-gold font-medium text-xs rounded-full hover:ring-2 hover:ring-rasbet-logo-gold focus:outline-none focus:ring-2 focus:ring-rasbet-logo-gold focus:bg-rasbet-logo-gold focus:text-white dark:focus:text-black"
          type="button"
          @click="openModalNotifs=true">
            <BellIcon class="h-8"/>
        </button>
      </div>
      
      <BaseModalNotifications :is-open="openModalNotifs" @close="openModalNotifs=false" />
    </div>
...
import { BellIcon, GiftIcon } from '@heroicons/vue/24/outline'
import BaseModalNotifications from './BaseModalNotifications.vue'
import BaseModalPromocoes from './BaseModalPromocoes.vue'

const openModalNotifs = ref<boolean>(false);
const openModalPromos = ref<boolean>(false);
