...
export const cartState = defineStore('cart', {
    ...
    actions: {
        ...
        async enviaAposta(token: string, carteira : boolean) : Promise<RasbetApiResponse> {
            const arrayResultados: ApostaItem[] = [];
            this.aposta.items.forEach((value,key) => {
                arrayResultados.push(value);
            })
            this.fetching = true;
            const res = await boletimPOST(token, arrayResultados, this.aposta.simples, this.aposta.valorTotal,carteira)
            this.fetching = false;
            return res;
        }
    }
